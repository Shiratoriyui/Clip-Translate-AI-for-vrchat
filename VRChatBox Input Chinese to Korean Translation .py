...
class TranslationBot:
    def __init__(self):
        ...
        self.current_prompt = "请将以下中文文本翻译成韩语。保持原意，使用自然的韩语表达。"
    ...
    "model": "deepseek-chat"
            ...
            sys.exit(1)
    def call_ai_api(self, text, prompt_type="translate_to_korean"):
        try:
            ...
            if prompt_type == "translate_to_korean":
                prompt = f"{self.current_prompt}\n\n原文：{text}\n\n以上中文翻译到韩语口语，作为朋友对话尽量不添加任何敬语，只输出韩语翻译结果，不要添加任何解释。"
            else:
    ...
    def translate_text(self, text):
        ...
        print("[翻译步骤] 第1步: 中文 → 韩语")
        korean = self.call_ai_api(text, "translate_to_korean")
        if not korean:
            print("[翻译错误] 第1步翻译失败")
            return None
        print(f"[翻译结果] 第1步完成，韩语翻译: {korean[:100]}...")
        time.sleep(0.5)
        print("[翻译步骤] 第2步: 韩语 → 中文")
        chinese_back = self.call_ai_api(korean, "translate_to_chinese")
        if not chinese_back:
            print("[翻译错误] 第2步翻译失败")
            return None
